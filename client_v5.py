            ...
            messages = [
                {
                    ...
                    "content": (
                        ...
                        "\n\n"
                        "# AVAILABLE TOOLS:\n"
                        "1. list_tables(db_type, connection_string) - List tables in SQLite or MySQL\n"
                        "   - SQLite example: list_tables('sqlite', '/path/to/db.sqlite')\n"
                        "   - MySQL example: list_tables('mysql', 'mysql://user:pass@host:port/dbname')\n"
                        "\n"
                        "2. run_sql(db_type, connection_string, query) - Execute SQL on SQLite or MySQL\n"
                        "   - Use the same connection_string format as list_tables\n"
                        "\n"
                        "3. run_python(code) - Execute Python code for calculations, data processing, web scraping, etc.\n"
                        "\n"
                        "# DATABASE SUPPORT:\n"
                        "You can work with both SQLite and MySQL databases.\n"
                        "- For SQLite: connection_string is just the file path\n"
                        "- For MySQL: connection_string format is 'mysql://username:password@hostname:port/database'\n"
                        "  Example: 'mysql://root:mypassword@localhost:3306/test_db'\n"
                        "\n"
